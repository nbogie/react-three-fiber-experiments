    ...
    return (
        <group>
            ...
            >
                ...
                <mesh scale={[0.3, 0.6, 0.3]} position={[0, 0.5, 0]}>
                    <coneGeometry />
                    <meshStandardMaterial color={"green"} />
                </mesh>

                <mesh scale={[0.11, 0.5, 0.11]} position={[0, -0.5, 0]}>
                    <cylinderGeometry />
...
function BaubleString({
    startRadius,
    endRadius,
    rise,
    vertOffset,
    areOn,
}: BaubleStringProps): JSX.Element {
    const baublePositions = useMemo(
        () => computeBaublePositions(startRadius, endRadius, rise, vertOffset),
        []
    );

    function computeBaublePositions(
        startRadius: number,
        endRadius: number,
        totalRise: number,
        vertOffset: number
        ...
        for (
            let angle = 0;
            angle <= totalAngle;
            angle += angleStep * randFloat(0.9, 1.1)
        ) {
            ...
            const randomOffset = new Vector3(1, 1, 1)
                .randomDirection()
                .multiplyScalar(randFloat(0.01, 0.02));
